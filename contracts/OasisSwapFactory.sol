...
contract OasisSwapFactory is IOasisSwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(OasisSwapPair).creationCode));
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        ...
        IOasisSwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'OasisSwap: FORBIDDEN');
        feeTo = _feeTo;
    }
